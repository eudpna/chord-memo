...
export default class Chords {
    ...
    getChordByName(name: string): null | ChordData {        
        // Major
        const m = name.match(/^(C#|C|D#|Db|D|Eb|E|F#|F|G#|Gb|G|A#|Ab|A|Bb|B)$/)
        if (m) return this.getChordByName(name + 'M')
...
export const getChordsOfInstrument = (instrument: 'guitar' | 'ukulele') => instrument === 'guitar' ? guitarChords : ukuleleChords

export const chordToName = (chord: ChordData): string => {
    if (chord.suffix === 'M') return chord.key
    else return chord.key + chord.suffix
}
